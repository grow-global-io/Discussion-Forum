...
const key =
  "KUkWogxJmmdUIHGl9B3QaCHU3100BO6H7Z6rdW0Daf6IGXWrUXXP6rZf7ZFZ5YSK14ZsqQoXjCctACDbcivASA==";
...
router.post("/add-like/:id", async (req, res) => {
  ...
  try {
    const updatedDocument = await updateDocument(id, userId);
    res.status(200).json({
      message: "User ID added to likedBy array.",
...
router.post("/add-thread/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "User ID added to threadPosts array.",
      document: updatedDocument,
    });
  } catch (error) {
    console.error(error);
...
router.delete("/remove-thread/:postId", async (req, res) => {
  ...
  try {
    ...
    if (index === -1) {
      return res
        .status(404)
        .json({ error: "Value not found in threadPosts array" });
    }
...
router.delete("/remove-like/:postId", async (req, res) => {
  ...
  try {
    ...
    if (index === -1) {
      return res
        .status(404)
        .json({ error: "Value not found in likedBy array" });
    }
...
async function updateDocumentThread(postId, userId, email) {
  ...
  const updatedData = {
    ...doc,
    threadPosts: [...doc.threadPosts, { userId, email }],
  };
  container.item(postId).replace(updatedData);
  return updatedData;
}
...
router.delete("/delete", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while deleting items." });
  }
});
