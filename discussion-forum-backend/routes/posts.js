...
router.delete('/:postId/remove-like', async (req, res) => {
...
async function updateDocument2(postId, userId) {
  ...
  const doc = resources[0]
  // Update the document in Cosmos DB
  const updatedData = { ...doc, comments: [...doc.comments, userId] };
  container.item(postId).replace(updatedData)
  return updatedData;
}
...
router.post('/create-thread/:id', async (req, res) => {
  try {
    ...
    const { id } = req.params;
    console.log(id)
    const updatedData = await updateDocument2(id,body);
